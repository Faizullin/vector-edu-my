import { ActionComponent, ActionRegistry } from "@/components/datatable";
...
export function LessonPageDrawer({
  ...
  const query = useQuery({
    ...
    queryFn: () => {
      return simpleRequest<LessonPageDocument[]>({
        url: `/lessons/pages`,
        method: "GET",
        params: {
          lesson_id: lessonId,
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: usedQueryKey });
      showToast("success", {
        message: "Lesson page deleted successfully",
      });
    },
  });
  const actions: ActionRegistry<LessonPageDocument> = useMemo(
    () => ({
      delete: {
        label: "Delete",
        callback: (item) => {
      ...
      openPages: {
        label: "Open Editor",
        callback: (item) => {
          openPostEditorMutation.mutate(item!.id, {
            onSuccess(data: any) {
              const postObj = data.post;
              window.open(
                getEditorLink(lessonId!, item!.id, postObj.id),
                "_blank"
              );
            },
          });
    ...
    [deleteMutation, moveUp, moveDown, openPostEditorMutation, lessonId]
  );
      ...
      col.display({
        ...
        cell: ({ row }) => {
          const resolvedActions: Array<
            ActionComponent<LessonPageDocument> & {
              id: string;
            }
          > = [];
          Object.entries(actions).forEach(([key, action]) => {
            resolvedActions.push({
              ...(action as ActionComponent<LessonPageDocument>),
              id: key,
            });
          });
          return (
            <div className="flex items-center gap-2">
              {resolvedActions.map((item) => (
                <Button
                  ...
                  variant="ghost"
                  size="sm"
                  onClick={() => item.callback(row.original)}
          ...
          <div className="flex-1 overflow-auto px-4 py-1 lg:flex-row lg:space-y-0 lg:space-x-12">
            <div className="flex items-center space-x-2">
              ...
              <Button
                ...
                size={"sm"}
              >
                <PlusIcon size={18} /> <span>Add</span>
              </Button>
            </div>
