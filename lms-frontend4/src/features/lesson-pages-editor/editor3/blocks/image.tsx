...
import { useBlockMediaAttachmentDialog } from "../hooks";
...
export const ImageBlock = createBlockSpec<ImageComponent>({
  ...
  render: ({ block }) => {
    ...
    const { showDialog } = useBlockMediaAttachmentDialog(block.id);
  ...
  sidebar: {
    render: ({ block, updateBlockField }) => {
      ...
      const { showDialog: showMediaDialog } = useBlockMediaAttachmentDialog(block.id);
          ...
          <Form {...formHook.form}>
            <div className="space-y-1">
              ...
              <div className="grid gap-2">
                <div
                  className={`relative flex items-center justify-center border-2 border-dashed rounded-lg h-40 ${!preview ? "border-gray-300 bg-gray-50" : "border-gray-200"
                    }`}
                >
                  {preview ? (
                    <div className="relative w-full h-full">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <input
                        ...
                        className={`absolute inset-0 w-full h-full opacity-0 cursor-pointer ${preview ? "pointer-events-none" : ""
                          }`}
                      />
                    )}
                  />
