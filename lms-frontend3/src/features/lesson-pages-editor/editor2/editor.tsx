...
const RenderEditor = () => {
  ...
  const { loadInitial, loadRelatedLessonPagesMutation, publishAndLoad } =
    useActionsContext();
  useEffect(() => {
    loadInitial();
  }, []);
  const { postObj, lessonObj, lessonPageObj } = useLoaderData({
          ...
          <div className="flex items-center justify-between max-w-3xl mx-auto">
            ...
            <div className="flex gap-2">
              <Button
                ...
                variant="outline"
                onClick={handleSave}
                disabled={!blocks.length || saveMuatation.isPending}
              ...
              <Button
                ...
                disabled={!blocks.length || publishMutation.isPending}
                className="flex items-center gap-1"
              >
                ...
                {blocks.map((block, index) => (
                  ...
                  >
                    <div
                      ...
                      className={`block-container relative p-2  border-b last:border-b-0 transition-colors ${
                        selectedBlockIdControl.state === block.id
                          ? "bg-blue-50"
                    ...
                    >
                      ...
                      >
                        ...
                        <div
                          className={`absolute left-0 top-1/2 -translate-y-1/2 -translate-x-full transition-opacity ${
                            selectedBlockIdControl.state === block.id ||
                            hoveredBlockIdControl.state === block.id
                              ? "opacity-100"
                              : "opacity-0"
                        ...
                        >
                          <Button
                            variant="ghost"
                            size="icon"
                            className="h-6 w-6 rounded-full border shadow-sm bg-white dark:bg-gray-900"
                            onClick={(e) => {
                              e.stopPropagation();
                              selectedBlockIdControl.setState(block.id);
                            }}
                          >
                          ...
                          <Button
                            variant="ghost"
                            size="icon"
                            className="h-6 w-6 rounded-full border shadow-sm text-red-500 bg-white dark:bg-gray-900"
                            onClick={(e) => {
                              e.stopPropagation();
                          ...
                          <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                              <Button
                                variant="outline"
                                size="icon"
                                className="h-8 w-8 rounded-full border-gray-200 shadow-sm"
                              >
